...
- **High — hash passwords instead of storing/comparing them in plaintext (§2.3).** `AclUser.passwords` holds exactly what was typed into `ACL SETUSER user >password`, compared with a plain `==`. Anything that can read process memory, a core dump, or (if `ACL LIST`/`GETUSER` output is ever extended to include raw passwords) the wire reply gets every credential in the clear. A standard salted hash (Argon2id, or at minimum SHA-256 with a per-user salt) with constant-time comparison is a well-understood, contained fix.
- **High — implement real per-command/per-key authorization, or stop advertising `all_commands`/`all_keys` as if they do something (§2.2/§4.2).** Right now `ACL SETUSER bob on >pw -@all` produces a user that, once authenticated, can run every command anyway — a deployment that believes it has restricted `bob` is not actually restricted. Either implement the check (gate `execute_command`'s dispatch on `auth_user`'s `AclUser.all_commands`/`all_keys`, and extend `ACL SETUSER`'s parser to handle `+@category`/`~pattern` tokens instead of silently accepting and ignoring them, §4.2) or fail loudly on unsupported ACL rules so operators don't deploy a false sense of restriction.
- **Medium — decide whether TLS is a real goal, and either wire it up or remove it (§2.4).** The `rustls`/`rcgen` handshake code is correct and complete but has zero callers — no `--tls-port` flag, no listener. If TLS support matters, the missing piece is a second listener (parallel to the existing `SO_REUSEPORT` plain-TCP one) that runs the accepted socket through `TlsSession::complete_handshake` before handing it to `handle_connection`. If it doesn't matter for the near term, removing the dead module avoids it silently bit-rotting as `rustls`/`rcgen` versions move on.
- **Medium — if kTLS is kept, add the missing `SOL_TLS`/`TLS_TX`/`TLS_RX` key-installation `setsockopt` call (§4.4).** `enable_ktls` currently only sets `TCP_ULP` to load the kernel's TLS module — it never installs the negotiated session key, so even a fully wired-up caller would get zero actual kernel offload today. This is a well-documented two-step Linux kTLS API; the first step (`TCP_ULP`) is already correct, only the second (installing the key material from the completed `rustls::ServerConnection`) is missing.
- **Low — expose jemalloc heap-profiling/dump capability, not just aggregate stats (§4.3)**, if deep memory-leak/fragmentation debugging in production ever becomes a need — `tikv-jemalloc-ctl` supports profiling hooks beyond the stats-only reads currently used.
