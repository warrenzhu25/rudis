...
- **High — add a graceful shutdown path.** There is no `SIGINT`/`SIGTERM` handler anywhere (§2.5), so a restart or deploy today is a hard kill: any buffered AOF chunk not yet flushed, any RDB save in progress, and any in-flight client request are simply dropped. A minimal fix: catch the signal on the main thread, broadcast a shutdown `ShardMessage` (or a shared `AtomicBool` each accept loop polls) to every shard, have each shard flush its `AofWriter` and stop accepting new connections, then join all threads before exiting.
- **Medium — reconcile this file's slot/shard model with live cluster migration.** `run_shard_worker` assigns shards statically at startup and never changes `num_shards`; meanwhile Component 04/11 already have real (if partially disconnected) live-migration machinery. Before slot migration can be considered a real feature end-to-end, this file's shard-count assumptions need to accommodate a shard's ownership actually changing at runtime, not just slot *state* changing.
- **Medium — give the 100ms/20ms/2s periodic tasks jitter or adaptive backoff.** All three are fixed-interval `monoio::time::sleep` loops; on a host running many shards, every shard's tasks tick in near-lockstep, which is a minor but avoidable source of correlated CPU bursts. Randomizing each task's phase (not its period) would smooth this out for free.
- **Low — reduce `SO_REUSEPORT` imbalance sensitivity.** The kernel's 4-tuple hash balancing is not perfectly even for a small number of long-lived connections (§5's "no connection-level backpressure" limitation); consider a periodic connection-count report per shard via `INFO`/logging so imbalance is at least observable, even without active rebalancing.
- **Low — document/guard the `BlockHub` exception's failure mode.** If `get_block_hub_for_port`'s `Mutex` were ever poisoned by a panic while held (e.g. inside `notify_list`'s call into `RudisTable`), every future blocking operation on that port would panic on `.lock().unwrap()`. Worth an explicit `catch_unwind` boundary around the critical section, or at least a comment flagging the risk, given how central this one lock is (§2.4).
