...
3. **Global, per-port shared statistics behind a lock.** `TieringStats` (21 atomic counters, including two configuration percentages) is stored in a process-wide `static TIER_STATS: RwLock<Option<HashMap<u16, Arc<TieringStats>>>>`, one entry per listening port, shared by every shard on that port. This is a real (small, read-mostly) synchronization point outside the shared-nothing data path, used purely for reporting (`TIER INFO`) and for the two threshold percentages, not for coordinating storage itself.
...
6. **`upload_threshold_pct` is configured and reported but not currently consumed by any gating decision.** `TieringStats` stores both `offload_threshold_pct` (default 60) and `upload_threshold_pct` (default 80), both settable via `CONFIG SET tiered-offload-threshold`/`tiered-upload-threshold` and both surfaced in `TIER INFO`. However, `Router::is_memory_constrained` — the function that decides whether a `GET` on a cold key should stream the value without promoting it to RAM — reads only `offload_threshold_pct`; `upload_threshold_pct` is not read anywhere in `src/router.rs`. Despite its doc comment ("stream cold reads without promotion when memory >= 80%"), the field is currently inert with respect to that behavior — the 60% `offload_threshold_pct` gate is what actually governs it.
